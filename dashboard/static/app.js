...
//  Live streaming with pan (drag) & zoom (scroll wheel)
// ============================================================
...
let windowMinutes = 1;        // visible time window
let autoScroll    = true;     // true = follow live data
const MAX_BUF     = 200000;   // ~28 hours at 0.5s

// Data buffers (always growing, trimmed at MAX_BUF)
const powerBuf    = { ts:[], input:[], output:[] };
const voltTempBuf = { ts:[], v12:[], t1:[], t2:[] };
const eventMarkers = [];      // [{x:Date, y:number, label:string}]
...
// Set the visible X range on a chart, using the zoom plugin API
// when available, otherwise falling back to direct scale options.
function setXRange(chart, min, max) {
    if (typeof chart.zoomScale === 'function') {
        // zoomScale updates internal plugin state AND calls chart.update
    ...
    options: {
        ...
        plugins: {
            ...
            tooltip: {
                ...
                callbacks: {
                    label(ctx) {
                        if (ctx.datasetIndex === 2 && ctx.raw && ctx.raw.label) {
                            return ctx.raw.label;
                        }
                        return `${ctx.dataset.label}: ${ctx.parsed.y.toFixed(1)} W`;
                    }
                }
            },
            zoom: {
...
function refreshCharts() {
    ...
    powerChart.data.datasets[2].data = eventMarkers;
    ...
    // First update data, then set visible range
    ...
    if (autoScroll) {
        ...
        setXRange(powerChart,    now - windowMs, now);
        setXRange(voltTempChart,  now - windowMs, now);
    }
}
...
function connect() {
    ...
    ws.onmessage = e => {
        ...
        else if (msg.type === 'error') console.warn('[PSU error]', msg.message);
    };
...
function onReading(d, newEvents) {
    ...
    // ── Transient event markers on power chart
    if (newEvents.length > 0) {
        newEvents.forEach(ev => {
            addEvent(ev);
            // Add marker at this timestamp with the current input power
            eventMarkers.push({
                x: ts,
                y: d.input_power || 0,
        ...
        // Keep markers buffer bounded
        while (eventMarkers.length > 500) eventMarkers.shift();
        updateEventCount();
    }
...
async function loadHistory() {
    ...
    try {
        const res  = await fetch(`/api/history?hours=${hours}&points=2000`);
        const rows = await res.json();

        if (rows.length > 0) {
            const hP = { ts:[], inp:[], out:[] };
            const hV = { ts:[], v12:[], t1:[], t2:[] };

            rows.forEach(r => {
                const ts = new Date(r.timestamp * 1000);
                hP.ts.push(ts);
                hP.inp.push(r.input_power);
                hP.out.push(r.output_power);
                hV.ts.push(ts);
                hV.v12.push(r.v12_voltage);
                hV.t1.push(r.temp1);
                hV.t2.push(r.temp2);
            });

            powerBuf.ts.unshift(...hP.ts);
            powerBuf.input.unshift(...hP.inp);
            powerBuf.output.unshift(...hP.out);
            voltTempBuf.ts.unshift(...hV.ts);
            voltTempBuf.v12.unshift(...hV.v12);
            voltTempBuf.t1.unshift(...hV.t1);
            voltTempBuf.t2.unshift(...hV.t2);
        }
        refreshCharts();
    } catch (e) {
    ...
    // Transient history
    try {
        const res2 = await fetch(`/api/transients?hours=${hours}`);
...
document.querySelectorAll('.time-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // Load more history for longer ranges
        if (minutes > 5) {
...
// Double-click on chart → resume live auto-scroll
['powerChart', 'voltTempChart'].forEach(id => {
    document.getElementById(id).addEventListener('dblclick', () => {
        autoScroll = true;
        windowMinutes = parseInt(document.querySelector('.time-btn.active')?.dataset.minutes || '1', 10);
        refreshCharts();
    });
});
...
document.getElementById('clearEvents').addEventListener('click', () => {
    ...
    document.getElementById('eventsList').innerHTML =
        '<div class="event-empty">No transient events detected. Monitoring...</div>';
    updateEventCount();
});
...
function setConnectionBadge(state) {
    ...
    fetch('/api/status').then(r => r.json()).then(s => {
        document.getElementById('demoBadge').style.display = s.demo_mode ? 'inline-block' : 'none';
    }).catch(() => {});
}
